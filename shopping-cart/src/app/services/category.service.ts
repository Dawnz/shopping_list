...
export class CategoryService {
  ...
  getCategoryById(id: string): Observable<APIResponse<Category>> {
    return this.http
      .get<APIResponse<Category>>(this.API_URL + '/' + id)
      .pipe(catchError(this._handleHttpErrors(new Category())));
  }

  createCategory(data: Category): Observable<APIResponse<Category>> {
    return this.http
      .post<APIResponse<Category>>(this.API_URL, data)
      .pipe(catchError(this._handleHttpErrors(new Category())));
  }
  ...
  updateCategory(
    ...
    return this.http
      .put<APIResponse<Category>>(this.API_URL + '/' + id, data)
      .pipe(catchError(this._handleHttpErrors(new Category())));
  }

  deleteCategory(id: string): Observable<APIResponse<Category>> {
    return this.http
      .delete<APIResponse<Category>>(this.API_URL + '/' + id)
      .pipe(catchError(this._handleHttpErrors(new Category())));
  }

  getCategoryByName(category: string): Observable<APIResponse<Category>> {
    return this.http
      .get<APIResponse<Category>>(this.API_URL + '/' + category)
      .pipe(catchError(this._handleHttpErrors(new Category())));
  }
}
